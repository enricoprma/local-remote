...
import { createApi, maxPointerDelta, maxScrollDelta, maxTextLength } from "../src/api";
...
test("moves the pointer with valid deltas", async () => {
    ...
    expect(input.movePointer).toHaveBeenCalledWith(maxPointerDelta, -maxPointerDelta);
});

test.each([
    { name: "positive delta above the limit", body: { dx: maxPointerDelta + 1, dy: 2 } },
    { name: "negative delta below the limit", body: { dx: 2, dy: -maxPointerDelta - 1 } },
...
test.each([
    ...
    { name: "excessively long text", body: { text: "a".repeat(maxTextLength + 1) } },

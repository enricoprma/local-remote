...
function setup(ui = false) {
  ...
  function send(
    ...
    (
      target ??
      (type === "pointerdown" || type === "lostpointercapture"
        ? touchArea
        : browser)
    ).dispatchEvent(event);
  }
...
test.each([7.9, 8, 15])(
  "classifies movement at distance %s and never clicks after crossing",
  (distance) => {
...
test.each(["pointercancel", "movement", "second finger", "capture loss"])(
  "%s cancels the old hold timer before another gesture starts",
  (ending) => {
    const { send, handlers } = setup();
    send("pointerdown");
...
test("the third finger starts drag immediately and release sends final movement before drag end", () => {
  ...
  handlers.onMove.mockImplementation((dx, dy) =>
    order.push(`move ${dx} ${dy}`),
  );
...
test.each([
  ...
  if (ending === "capture loss") send("lostpointercapture");
  if (ending === "blur" || ending === "pagehide")
    browser.dispatchEvent(new Event(ending));
  if (ending === "hidden") {
...
test.each([1, 2, 3])(
  "release of drag finger %s ends drag and blocks the remaining contacts",
  (id) => {
...
test.each([1, 2, 3])(
  "cancellation of drag finger %s ends drag without final movement",
  (id) => {
    const { send, handlers, drag } = setup();
    drag();
...
test.each(["movement", "hold", "scroll"])(
  "third finger can start dragging after %s",
  (prior) => {
    const { send, handlers } = setup();
    send("pointerdown", 1);
...
test.each([1, 2])(
  "right-click waits for both releases, starting with finger %s",
  (first) => {
    const { send, handlers } = setup();
    send("pointerdown", 1);
    send("pointerdown", 2);
...
test.each([
  ...
  send(
    ending === "cancel" ? "pointercancel" : "pointerup",
    2,
...
test.each(["horizontal", "third finger", "cancel"])(
  "%s suppresses two-finger tap",
  (change) => {
    const { send, handlers } = setup();
    send("pointerdown", 1);
    send("pointerdown", 2);
...
test("a failed capture cannot arm a hold", () => {
  ...
  touchArea.setPointerCapture.mockImplementation(() => {

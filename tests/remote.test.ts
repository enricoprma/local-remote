...
test("concurrent callers preserve command order while combining adjacent moves", async () => {
  ...
  const completed = Promise.all([
    ...
    remote.movePointer(4, 5),
    remote.movePointer(-2, 1),
    remote.action("end-drag"),
    remote.action("right-click"),
    remote.scroll(3),
    remote.typeText("hello"),
    remote.action("enter"),
  ]);
...
test("movement never combines across drag boundaries or discrete commands", async () => {
  ...
  const completed = Promise.all([
    ...
    remote.movePointer(4, 5),
    remote.action("end-drag"),
    remote.movePointer(6, 7),
    remote.action("click"),
    remote.movePointer(8, 9),
    remote.scroll(2),
    remote.movePointer(10, 11),
  ]);
  const commands = [
    ["pointer", { dx: 2, dy: 3 }],
    ["action", { action: "start-drag" }],
    ["pointer", { dx: 4, dy: 5 }],
    ["action", { action: "end-drag" }],
    ["pointer", { dx: 6, dy: 7 }],
    ["action", { action: "click" }],
    ["pointer", { dx: 8, dy: 9 }],
    ["scroll", { dy: 2 }],
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    expect(JSON.parse(String(fetch.mock.calls[i][1]?.body))).toEqual(
      commands[i][1],
    );
    responses[i](new Response(null, { status: 204 }));
  }
...
test.each([401, 500, "network"] as const)(
  "%s failure drops queued movement but still sends release and allows recovery",
  async (failure) => {
...
test("an aborted request also advances the queue to the release", async () => {
  ...
  const timeout = vi
    ...
    .mockReturnValue(timeoutSignal.signal);
  try {
    const remote = await import("../client/services/remote");
    const completed = Promise.allSettled([
      remote.action("start-drag"),

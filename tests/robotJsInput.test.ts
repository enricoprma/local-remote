...
test.each([false, true])(
  "drag actions reach native input through HTTP (lost start response: %s)",
  async (loseResponse) => {
    vi.useRealTimers();
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn<typeof fetch>(async (path, options) => {
    ...
    try {
      const results = await Promise.allSettled([
        remote.action("start-drag"),
        remote.movePointer(6, 3),
      ...
      if (loseResponse) {
        ...
        expect(robot.mouseToggle.mock.invocationCallOrder[0]).toBeLessThan(
          robot.dragMouse.mock.invocationCallOrder[0],
        );
        expect(robot.dragMouse.mock.invocationCallOrder[0]).toBeLessThan(
          robot.mouseToggle.mock.invocationCallOrder[1],
        );
      }
...
test("down/up are idempotent and movement chooses the native operation", async () => {
  ...
  expect(robot.moveMouse.mock.calls).toEqual([
    [103, 196],
    [101, 202],
  ]);
...
test("a missing remote release expires after inactivity, with movement extending it", async () => {
  const { robotJsInput: input, pointerIdleTimeoutMs } =
    await import("../src/robotJsInput");
  input.executeAction("start-drag");
...
test("a failed release keeps the held state and retries native release", async () => {
  const { robotJsInput: input } = await import("../src/robotJsInput");
  input.executeAction("start-drag");
  robot.mouseToggle.mockImplementationOnce(() => {
...
test("a failed drag movement releases held input before reporting the error", async () => {
  const { robotJsInput: input } = await import("../src/robotJsInput");
  input.executeAction("start-drag");
  robot.dragMouse.mockImplementationOnce(() => {
...
test.each(["click", "right-click"] as const)(
  "%s goes through action mapping without leaving held state",
  async (action) => {
    const { robotJsInput: input } = await import("../src/robotJsInput");
    input.executeAction("start-drag");
    ...
    input.movePointer(1, 2);
    expect(robot.mouseToggle).toHaveBeenLastCalledWith("up", "left");
    expect(robot.mouseClick).toHaveBeenCalledWith(
      action === "click" ? "left" : "right",
    );

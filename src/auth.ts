    ...
    return {
        ...
        pair(credential) {
            if (pairingWindow === null) {
                console.warn(
                    "[auth] Pairing rejected: no pairing window open",
                );
            ...
            if (
                pairingWindow.expiresAt <= Date.now()
            ) {
                pairingWindow = null;

                console.warn(
                    "[auth] Pairing rejected: pairing window expired",
                );
            ...
            if (
                ...
                console.warn(
                    "[auth] Pairing rejected: pairing limit reached",
                );
            ...
            const matchesSecret =
                credential === pairingWindow.secret;

            const matchesCode =
                credential === pairingWindow.code;

            if (!matchesSecret && !matchesCode) {
                console.warn(
                    "[auth] Pairing rejected: credential does not match",
                );
